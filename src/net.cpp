...
Net::Net(unsigned int nInputs, unsigned int nHiddenLayers, unsigned int nHiddenNeurons, unsigned int nOutput){
	...
	for(unsigned int l = 1; l < nHiddenLayers; l++){
		for(unsigned int i = 0; i < nHiddenNeurons; i++){
...
void Net::openNet(char* arquiv){
	...
	for(int i = 0; i < nHiddenNeurons; i++){
		...
		aux->clear();
		//ler os pesos
		for(unsigned int j = 0; j < this->nInputs; j++){
			double peso;
...
void Net::rerandom(){
		...
		for(unsigned int l = 0; l < nHiddenLayers; l++){
			for(unsigned int i = 0; i < nHiddenNeurons; i++){
...
void Net::activateLayers(vector<double>* inputs, vector<double>* poutputs){
	...
	//camada de entrada
	for(unsigned int n = 0; n < nHiddenNeurons; n++){
...
unsigned int Net::getNInputs(){
    return nInputs;
}
unsigned int Net::getNHiddenLayers(){
    return nHiddenLayers;
}
unsigned int Net::getNHiddenNeurons(){
    return nHiddenNeurons;
}
unsigned int Net::getNOutput(){
    return nOutput;
}
...
void Net::imprimeRede(){
    for(unsigned int i = 0; i < nHiddenLayers+1; i++){
        imprimeCamada(i);
    }
}

void Net::configureOutputs(vector<double> poutputs){
	for(unsigned int i = 0; i < nOutput; i++){
		layers[nHiddenLayers][i].addict(poutputs[i]);
	}
}

void Net::parse_chromosome(Chromosome *cromosome){
	unsigned int chromosome_index = 0;
	
	// Passar os pesos da primeira camada
	// Para cada neuronio da primeira camada
	for(unsigned int i = 0; i < nHiddenNeurons; i++){
	...
	// Passar os pesos das outras camadas escondidas
	// Para cada camada escondida
	for(unsigned int i = 1; i < nHiddenLayers; i++){
	...
	// Passar os pesos da camada de saida
